...
from kernels.mmq_q6_k_q8_1 import mmq_q6_k_q8_1
from kernels.cpu_impls.mmq_q6_k_q8_1_cpu import mmq_q6_k_q8_1_cpu

mnk_list = []
for m_pow in range(0, 6, 2):
    for n_pow in range(0, 6, 2):
        for k_pow in range(8, 11, 1):
...
for M, N, K in mnk_list:
    ...
    quant_A = quantize_to_q6_k(float_A)
    quant_B = quantize_to_q8_1(float_B)

    C_cpu = mmq_q6_k_q8_1_cpu(quant_A, quant_B, M, N, K)
    C_triton = mmq_q6_k_q8_1(quant_A.to("cuda:0"), quant_B.to("cuda:0"), M, N, K)
